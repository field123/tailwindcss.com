...
The palette even includes 5 different shades of gray now, so you can choose "zinc gray" if you want something really cool, or go all the way to "warm gray" for something with a lot more brown in it.
...
```html
<button
  class="... focus:outline-none focus:ring-2 focus:ring-zinc-300 focus:ring-opacity-50"
>
  <!-- ... -->
</button>
```
...
```html
<button
  class="... focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-300 focus:ring-opacity-50"
>
  <!-- ... -->
</button>
```
...
We've also extended the `inset` (that's top/right/bottom/left for you dinosaurs) and `tranzinc` plugins to include the full spacing scale, so now you can do things like this:
...
```html
<div class="group ...">
  <span class="group-hover:text-zinc-600 ...">Da ba dee da ba daa</span>
</div>
```
