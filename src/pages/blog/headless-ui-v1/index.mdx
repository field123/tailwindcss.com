  ...
  return (
    <RadioGroup value={plan} onChange={setPlan}>
      ...
      <RadioGroup.Option value="startup">
        {({ checked }) => (
          <span className={checked ? 'bg-zinc-200' : ''}>Startup</span>
        )}
      </RadioGroup.Option>
      <RadioGroup.Option value="business">
        {({ checked }) => (
          <span className={checked ? 'bg-zinc-200' : ''}>Business</span>
        )}
      </RadioGroup.Option>
      <RadioGroup.Option value="enterprise">
        {({ checked }) => (
          <span className={checked ? 'bg-zinc-200' : ''}>Enterprise</span>
        )}
      </RadioGroup.Option>
    </RadioGroup>
  )
...
<template>
  ...
  <TransitionRoot :show="isOpen">
    ...
    <TransitionChild
      ...
      duration-300
      transform"
      enter-from="-tranzinc-x-full"
      enter-to="tranzinc-x-0"
      leave="transition"
      ease-in-out
      duration-300
      transform"
      leave-from="tranzinc-x-0"
      leave-to="-tranzinc-x-full"
    >
      <!— … —>
    </TransitionChild>
  </TransitionRoot>
