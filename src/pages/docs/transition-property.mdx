...
<Example hint="Hover the button to see the expected behaviour">
  <div class="flex justify-around text-white text-sm font-bold leading-6">
    <button class="px-4 py-2 font-semibold text-sm bg-zinc-500 text-white rounded-md shadow-sm hover:-tranzinc-y-1 hover:scale-110 hover:bg-indigo-500 ease-in-out delay-150 duration-300">
      Save Changes
    </button>
  </div>
</Example>
...
```html
<button class="**transition** ease-in-out delay-150 bg-zinc-500 hover:-tranzinc-y-1 hover:scale-110 hover:bg-indigo-500 duration-300 ...">
  Save Changes
</button>
```
...
```html
<button class="transition transform hover:-tranzinc-y-1 **motion-reduce:transition-none motion-reduce:hover:transform-none** ...">
  Hover me
</button>
```
