...
sidebar_label: Integration Payload
---
...
The following example payload [observes](observable-events.md) the `order.paid` event. The configured URL receives the following payload.
...
| `type` | [`string`](observable-events.md) | The [observable event](observable-events.md) that triggered this event. |
| `data` | `object` | The resource affected by this event. |


### Example: Typical payload

The following example payload [observes](observable-events.md) the `cart.updated` event. The configured queue receives the following message payload:
