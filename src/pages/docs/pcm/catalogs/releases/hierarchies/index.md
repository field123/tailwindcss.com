...
sidebar_label: Catalog by Shopper Context
---

Use the Catalog View API to create, manage, and publish catalogs. You can also define catalog rules so that you can show catalogs with different pricing or different products to preferred customers or channels.

You can use the Catalog View service with either the `pcm/products` resource or the `v2/products` resource.

## Catalog View with `pcm/products`
...
## Catalog View with `v2/products`

You can use the Catalog View with legacy catalog products using the `v2/products` API to retrieve products associated with a legacy catalog. The hierarchies come from the brands, categories, and collections that are associated with the products. The prices are defined in the products, which means that the Price Book API is not supported with the legacy `v2/products` resource.
...
- To create a catalog rule for all shoppers, use the `catalogs/rules` API and set `catalog_id=legacy` and leave the other optional catalog rule fields empty.
