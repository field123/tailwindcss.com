...
export function CodeWindow({ children, className, border = true }) {
  return (
    <div
      className={clsx(
        'relative overflow-hidden shadow-xl flex bg-zinc-800 h-[31.625rem] max-h-[60vh] sm:max-h-[none] sm:rounded-xl lg:h-[34.6875rem] xl:h-[31.625rem] dark:bg-zinc-900/70 dark:backdrop-blur dark:ring-1 dark:ring-inset dark:ring-white/10',
        className
      )}
    >
      <div className="relative w-full flex flex-col">
        <div className={clsx('flex-none', border && 'border-b border-zinc-500/30')}>
    ...
    <div className="w-full flex-auto flex min-h-0 overflow-auto">
      <div ref={ref} className="w-full relative flex-auto">
        <pre className="flex min-h-full text-sm leading-6">
          <div
            aria-hidden="true"
            className="hidden md:block text-zinc-600 flex-none py-4 pr-4 text-right select-none"
            style={{ width: 50 }}
          >
            {lineNumbers}
          </div>
          <code className="flex-auto relative block text-zinc-50 pt-4 pb-4 px-4 overflow-auto">
        ...
        <div className="w-full relative flex-auto">
          ...
          >
            {showLineNumbers && (
              <div
                aria-hidden="true"
                className="hidden md:block text-zinc-600 flex-none py-4 pr-4 text-right select-none w-[3.125rem]"
              >
            ...
            <code
              className={clsx(
                'flex-auto relative block text-zinc-50',
                {
