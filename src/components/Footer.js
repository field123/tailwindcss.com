...
export function Footer({ children, previous, next }) {
  return (
    <footer className={clsx('text-sm leading-6', previous || next ? 'mt-12' : 'mt-16')}>
      {(previous || next) && (
        <div className="mb-10 text-zinc-700 font-semibold flex items-center dark:text-zinc-200">
          {previous && (
            <Link href={previous.href}>
              <a className="group flex items-center hover:text-zinc-900 dark:hover:text-white">
                <svg
                  viewBox="0 0 3 6"
                  className="mr-3 w-auto h-1.5 text-zinc-400 overflow-visible group-hover:text-zinc-600 dark:group-hover:text-zinc-300"
                >
                  <path
                    d="M3 0L0 3L3 6"
                    fill="none"
          ...
          {next && (
            <Link href={next.href}>
              <a className="group ml-auto flex items-center hover:text-zinc-900 dark:hover:text-white">
                {next.shortTitle || next.title}
                <svg
                  viewBox="0 0 3 6"
                  className="ml-3 w-auto h-1.5 text-zinc-400 overflow-visible group-hover:text-zinc-600 dark:group-hover:text-zinc-300"
                >
                  <path
                    d="M0 0L3 3L0 6"
                    fill="none"
      ...
      <div className="pt-10 pb-28 border-t border-zinc-200 sm:flex justify-between text-zinc-500 dark:border-zinc-200/5">
        <div className="mb-6 sm:mb-0 sm:flex">
          <p>Copyright &copy; {new Date().getFullYear()} Tailwind Labs Inc.</p>
          <p className="sm:ml-4 sm:pl-4 sm:border-l sm:border-zinc-200 dark:sm:border-zinc-200/5">
            <Link href="/brand">
              <a className="hover:text-zinc-900 dark:hover:text-zinc-400">Trademark Policy</a>
            </Link>
          </p>
        </div>
        {children ? (
          ...
          <div className="flex space-x-10 text-zinc-400 dark:text-zinc-500">
            <a
            ...
            <a href="/discord" className="hover:text-zinc-500 dark:hover:text-zinc-400">
