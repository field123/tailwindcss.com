...
export function IconContainer({ as: Component = 'div', className = '', light, dark, ...props }) {
  return (
    <Component
      className={`w-16 h-16 p-[0.1875rem] rounded-full ring-1 ring-zinc-900/10 shadow overflow-hidden ${className}`}
      {...props}
    >
      {light && (
        <div
...
export function BigText({ className = '', ...props }) {
  return (
    <p
      className={`mt-4 text-3xl sm:text-4xl text-zinc-900 font-extrabold tracking-tight dark:text-zinc-50 ${className}`}
      {...props}
    />
  )
}
...
export function InlineCode({ className = '', ...props }) {
  return (
    <code
      className={`font-mono text-zinc-900 font-medium dark:text-zinc-200 ${className}`}
      {...props}
    />
  )
}
