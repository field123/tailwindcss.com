...
function TabItem({ children, selectedIndex, myIndex, marker }) {
  ...
  return (
    ...
    >
      ...
      {marker === 'close' && (
        <svg viewBox="0 0 4 4" className="ml-2.5 flex-none w-1 h-1 text-zinc-500 overflow-visible">
      ...
      {!isSelected && (
        <TabAdornment
          className={clsx(
            'bg-zinc-700/50 border-y border-zinc-500/30',
      ...
      {edges.trailing === 'normal' && (
        <TabAdornment className="inset-y-px border-r border-zinc-200/5 z-20" />
      )}
...
export function SnippetGroup({ children, actions }) {
  ...
  return (
    <Tab.Group
      ...
      className="not-prose bg-zinc-800 rounded-xl shadow-md"
    >
      <div className="flex">
        <Tab.List className="flex text-zinc-400 text-xs leading-6 overflow-hidden rounded-tl-xl pt-2">
        ...
        <div className="flex-auto flex pt-2 rounded-tr-xl overflow-hidden">
          <div
            className={clsx(
              'flex-auto flex justify-end bg-zinc-700/50 border-y border-zinc-500/30 pr-4',
      ...
      <Tab.Panels className="flex overflow-auto">
        {children.map((child) => (
          <Tab.Panel
            ...
            className="flex-none min-w-full p-5 text-sm leading-6 text-zinc-50 ligatures-none"
          >
