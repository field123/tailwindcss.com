...
export default function PostItem({ title, category, slug, date, children, wide = false }) {
  return (
    ...
    >
      <h3 className="mb-4 text-xl text-zinc-900 tracking-tight font-bold dark:text-zinc-200">
        <Link href={`/blog/${slug}`}>
          <a>{title}</a>
        </Link>
      </h3>
      <div className="mb-6 prose prose-zinc dark:prose-dark">{children}</div>
      <div className="mt-auto flex flex-row-reverse items-center justify-end">
        <dl>
          ...
          <dd
            className={clsx('text-sm leading-6 dark:text-zinc-400', {
        ...
        <svg
          ...
          height="2"
          fill="currentColor"
          className={clsx('mx-4 text-zinc-700', { 'lg:hidden': wide })}
        >
